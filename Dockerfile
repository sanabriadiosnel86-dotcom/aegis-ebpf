...
# The built dashboard on its own, for `make build-web`.
FROM scratch AS web
...
# The dashboard is bundled so the agent can serve it with
# `-web-dir /usr/share/aegis/web`.
COPY --from=web-build /web/dist /usr/share/aegis/web
EXPOSE 8080
